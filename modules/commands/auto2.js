  ...
  try {
    ...
    if (isURL(str)) {
      ...
      /* TỰ ĐỘNG TẢI ẢNH HOẶC VIDEO TIKTOK */ 
          ...
          o.api.sendMessage({body: `${head('TIKTOK')}\n🧔Author: ${json.author.nickname}\n📝Tiêu Đề : ${json.title} \n\n📌thả cảm xúc vào đây để tải nhạc`, attachment },o.event.threadID,(error, info) => {
    ...
    /*  else if (/imgur\.com/.test(str)) {
      ...
      /*AUTODOWN CAPCUT VIIDEO */
                ...
                var res = (await axios.get(`https://gaudev-api.onrender.com/capcut?url=${str}`))
   ...
   /*   else if(/catbox\.moe/.test(str)){
      ...
      const res = (await axios.get(`https://0b1f97ef-2687-473d-9270-c18c3f0c2cdf-00-3sgeg370klrjz.sisko.replit.dev/tw/download?link=${str}`)).data
      let attachment = [];
