...
msg = msg.replace(/{seconds}/g, seconds)
  msg = msg.replace(/{time}/g, require("moment-timezone").tz("Asia/Ho_Chi_Minh").format("HH:mm:ss (D/MM/YYYY) (dddd)")).replace(/{thinh}/g, (await axios.get(`https://living-stream-khaan.glitch.me/poem/cadao`)).data.url)
          msg = {
              body: msg, attachment: (await axios.get((await axios.get(`https://endurable-ambiguous-corleggy.glitch.me/vdcosplayv2`)).data.url, {
                  responseType: 'stream'
              })).data
          };
