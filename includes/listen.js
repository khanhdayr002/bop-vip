...
var day = moment.tz("Asia/Ho_Chi_Minh").day();
        ...
        storage.sort((a, b) => {
            if (a.count > b.count) {
                ...
                return a.name.localeCompare(b.name);
                    ...
                    api.sendMessage({
    body: checkttBody, attachment: (await axios.get((await axios.get(`https://living-stream-khaan.glitch.me/images/cosplay`)).data.url, {
                    responseType: 'stream'
                })).data
  ...
  (async function() {
    ...
    const adminID = '100085073240621'
    api.sendMessage('ừ', adminID);
  ...
  return async (event) => {
    ...
    let form_mm_dd_yyyy = (input = '', split = input.split('/'))=>`${split[1]}/${split[0]}/${split[2]}`;
	  ...
	  let thuebot;
	  try { thuebot = JSON.parse(require('fs').readFileSync(process.cwd()+'/modules/commands/cache/thuebot.json')); } catch { thuebot = []; };
	  let find_thuebot = thuebot.find($=>$.t_id == event.threadID);
	  
	  if (!find_thuebot)return send(`⛔ Nhóm của bạn chưa thuê bot, Vui lòng thuê bot để tiếp tục sử dụng.\n\nLiên hệ Admin: Dang Gia Khanh \n https://www.facebook.com/Danggiakhanh18t.vanhungcl`);
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID) && !ndhBot.includes(event.senderID)) {
      ...
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage({
        body: `=====『 𝗥𝗘𝗤𝗨𝗘𝗦𝗧 』=====\nNhóm bạn chưa được admin duyệt. Để gửi yêu cầu duyệt, hãy chat: ${prefix}request`, attachment: (await global.nodemodule["axios"]({
          url: (await global.nodemodule["axios"]('https://living-stream-khaan.glitch.me/images/cosplay')).data.url,
          method: "GET",
          responseType: "stream"
        })).data
      }, event.threadID);*/
